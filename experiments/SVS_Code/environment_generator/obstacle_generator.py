...
if len(sys.argv) >= 3:
    DENSITY_MULTIPLIER = float(sys.argv[2])
else:
...
class Obstacle:
    def __init__(self, config):
        ...
        # fix sphere at 1000.0
        while self.trajectory.pos[0, :][0] == 1000.0:
            self.trajectory = Trajectory(config, self.traj_type)
...
class ObstacleGroup:
    def __init__(self, global_config, local_config, name="object"):
        ...
        self.num_objects = int(np.ceil(self.area * (self.config['density'] * DENSITY_MULTIPLIER)))
